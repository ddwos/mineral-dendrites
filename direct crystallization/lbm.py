...
def initialize_lbm(consA, consB, DA, DB):
    ...
    return feqA, feqB, finA, finB, omegaA, omegaB, d, q, c, w


def mass_conservation(consA, consB, consA_old, consB_old, finA, finB):
...
def collision_step(w, consA, consB, omegaA, omegaB, finA, finB):
...
def apply_boundary_conditions(foutA, foutB, finA, finB, l):
    ...
    return foutA, foutB, finA, finB


def streaming_step(q, foutA, foutB, finA, finB, c):
    '''Streaming step of the Lattice-Boltzmann method'''

    for i in range(q):  # Streaming step.
        finA[i, :, :] = roll_concatenate(
            roll_concatenate(foutA[i, :, :], c[i, 0], axis=0), 
            c[i, 1], axis=1)
        finB[i, :, :] = roll_concatenate(
            roll_concatenate(foutB[i, :, :], c[i, 0], axis=0), 
            c[i, 1], axis=1)
